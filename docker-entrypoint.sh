...
case "$*" in
    ...
    *)
        ...
        if [ "$DJANGO_ENV" = "production" ]; then
            ...
            # Tailwind build INPUTS live in assets/ (outside STATICFILES_DIRS)
            # precisely so collectstatic never sees them: collecting input.css
            # made manifest post-processing chase @import "tailwindcss" as a
            # static file ('src/tailwindcss'), which crashed collectstatic and
            # took prod down on 2026-07-29. Keep build sources out of static/.
