...
def calculate_full_dna(csv_file_content: str, user=None, session_key=None, progress_cb=None):
    try:
        ...
        if read_df.empty:
            raise ValueError("No books found with 'read' status in your CSV. Make sure your export includes books you've finished reading.")
                    ...
                    for future in as_completed(futures):
                        ...
                        if result:
                            ...
                            if "page_count" in result:
                                ...
                                Book.objects.filter(pk=book_pk).update(page_count=result["page_count"])
                            if "publish_year" in result:
                                read_df.at[idx, "Original Publication Year"] = result["publish_year"]
                                Book.objects.filter(pk=book_pk).update(publish_year=result["publish_year"])
                            if "average_rating" in result:
                                read_df.at[idx, "Average Rating"] = result["average_rating"]
                                Book.objects.filter(pk=book_pk).update(average_rating=result["average_rating"])
                            enriched_count += 1
                        if progress_cb:
