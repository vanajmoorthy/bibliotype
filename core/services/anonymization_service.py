...
def anonymize_session(anonymous_session):
    ...
    AnonymizedReadingProfile.objects.create(
        ...
        average_rating=(
            dna.get("average_rating_overall") if isinstance(dna.get("average_rating_overall"), (int, float)) else None
        ),
...
def batch_anonymize_expired_sessions():
    ...
    expired_sessions = AnonymousUserSession.objects.filter(expires_at__lt=timezone.now(), anonymized=False)
    ...
    AnonymousUserSession.objects.filter(anonymized=True, expires_at__lt=cleanup_date).delete()
