...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--num-users", type=int, default=5, help="Number of test users to create")
        parser.add_argument("--books-per-user", type=int, default=30, help="Number of books per user")
    ...
    def handle(self, *args, **options):
        ...
        if len(books) < 20:
            self.stdout.write(
                self.style.ERROR(f"Not enough books in database. Found {len(books)} books. Need at least 20.")
            )
        ...
        for i in range(1, num_users + 1):
            ...
            if DjangoUser.objects.filter(username=username).exists():
                ...
                user = DjangoUser.objects.create_user(
                    username=username, email=f"reader{i}@test.com", password=password, first_name=f"Test Reader {i}"
                )
