...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--password", type=str, default="testpass123", help="Password for created users (default: testpass123)"
        )
        parser.add_argument("--skip-existing", action="store_true", help="Skip users that already exist")
        parser.add_argument(
            "--pattern",
            type=str,
            ...
            if user_exists:
                ...
                try:
                    user = User.objects.create_user(
                        username=username, email=email, password=password, first_name=username.replace("_", " ").title()
                    )
